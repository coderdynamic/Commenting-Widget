( function(){
	...
	function Comment(userName,text,votes,commentList,hours,mins){
	...
	Comment.prototype.reply = function(userName,text){
		var d2 = new Date()
		var reply = new Comment(userName,text,0,[],d2.getHours(),d2.getMinutes());
	...
	function createComment(userName,text,hours,mins,votes){
		var comment = new Comment(userName,text,votes,[],hours,mins);
		comment.save()
		return comment
	}
	...
	function showComments(commentList){
		...
		for(var i=0;i<commentList.length;i++){
			var comment = new Comment(commentList[i].userName,commentList[i].text,commentList[i].votes,commentList[i].commentList,commentList[i].hours,commentList[i].mins)	
	...
	function createLi(comment,index){
		...
		var votes = document.createTextNode(comment.hours+":"+comment.mins+"  "+"Votes:"+comment.votes);
		...
		postReplyBtn.onclick = function(){
			...
			var d1 = new Date()
			var reply = new Comment(user,content,0,[],d1.getHours(),d1.getMinutes())
	...
	document.getElementById('post').addEventListener('click',function(){
		...
		createComment(userName,content,d.getHours(),d.getMinutes(),0)
	})
